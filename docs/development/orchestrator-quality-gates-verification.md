...
| Evidence | Result |
| --- | --- |
| Quality Gates contracts, canonicalization, total iterative lineage, evaluator propagation, contradictions, decisions, manifest, hostile boundary, fingerprint evidence and isolation | 190 passed; one pre-existing Pydantic warning |
...
| Current full suite | 459 passed; nine pre-existing warnings |
...
Ordinary leaves declare exact expected constructed-tree paths; the catalog validator calculates actual paths and requires exact equality plus agreement with the normative source label. `batch_id`, `result_id`, `claim_id`, `evidence_id`, `assumption_id`, `limitation_id` and `contradiction_id` retain independently declared minimal reference closures that are compared with actual before/after data. Gate-coherent coupled evidence is declared separately for status/materiality combinations. Deterministic mutation signatures contain independently normalized endpoint pairs, unordered before/after values, actual changed paths and canonical semantic ordering targets with raw sequence endpoints where applicable. Endpoint ordering is lexical and direction-invariant, so rebuilt or reversed evidence has the same signature; no case name, display label, alias, caller raw key or category participates in identity.

Results, claims, evidence, assumptions, limitations and contradictions use stable record identities to prove exactly one complete element is added or removed while retained content remains equal. `contradiction.left` and `.right` structural coverage is resolved from the actual isolated descendant mutation paths, with one real case per descendant. Every normalization and side-position case selects one of fourteen closed `OrderingTarget` adapters. Each immutable adapter exclusively owns its canonical semantic/raw path, builder, constructed-contract projection, stable element identity, normalization semantics and fingerprint expectation. Controlled witness factories bind the exact raw sequences to that adapter; registry validation rejects duplicate target, semantic and raw paths, while unknown strings, semantic aliases, forged sources and builder/projection mismatches cannot count as evidence.

Ordering validation reaches stable guards in declared order: fixed adapter and witness association, length, distinctness, equal multiset, real change, exact reversal, unrelated raw-field exclusion, evaluator validity and fingerprint behavior. Twenty-two individually named catalog-level negative controls match exact error identifiers for leaf mislabelling; same-direction, reversed, independently rebuilt, membership, ordering and identity duplicate mutations; identity rename as membership; missing structural descendants; unknown/aliased/forged/mismatched ordering targets; identical, short, duplicate, non-reversed and membership-changing sequences; unrelated raw changes; unused witnesses; legacy element-count metadata; and duplicate semantic adapter registration. A positive neighbor proves genuinely different mutations retain distinct signatures.
