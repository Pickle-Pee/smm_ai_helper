...
- [x] 11.9 Add an independently declared, evaluator-valid fingerprint participation catalog with exact actual-path matching for isolated leaves and reference closures, actual before/after mutation-signature uniqueness, stable-identity membership proofs, structural coverage derived from actual leaf mutations, shared-builder pre-normalization ordering witnesses, twelve catalog-level negative controls, derived-field exclusion and all four unchanged fixed vectors.
