...
- [x] 11.9 Add an independently declared, evaluator-valid fingerprint participation catalog with exact actual-path matching for isolated leaves and reference closures, direction-invariant actual mutation-signature uniqueness, stable-identity membership proofs, structural coverage derived from actual leaf mutations, fourteen closed semantic ordering adapters with builder/projection binding and alias rejection, twenty-two individually named catalog-level negative controls with exact stable errors and ordered invariant reachability, derived-field exclusion and all four unchanged fixed vectors.
