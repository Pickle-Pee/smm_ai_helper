...
- [x] 11.9 Add an independently declared fingerprint participation matrix using evaluator-valid purpose-specific fixtures: isolated legal mutations for independently mutable leaves, asserted minimal reference-closed mutations for identities, descendant-leaf coverage for structurally inseparable records, genuine valid membership changes, non-vacuous two-element order rules, derived-field exclusion and all four unchanged fixed vectors.
