...
No caller supplies structural validity, `GateOutcome`, execution readiness, synthesis eligibility, contradiction state/precedence, propagated claim context, exclusion records, manifest content or batch fingerprint.

Every public caller-owned contract uses a controlled constructor boundary. Missing required fields, unknown fields, duplicate positional/keyword representations, and attempts to supply derived/output-only fields raise `QualityGateContractError`, never autogenerated argument-binding `TypeError`. Valid positional construction remains supported in the declared field order. Validation rejects hostile values by exact type before invoking caller-defined behavior. Frozen/slotted dataclass equality, hashing where available, defensive copying and deep immutability remain unchanged; unrelated programmer defects and `BaseException` are not normalized.
...
| `PropagatedClaimContext` | `batch_id: BatchId`; `batch_fingerprint: BatchFingerprint`; `claim_id: ClaimId`; `effective_confidence: Confidence`; `evidence_ids: tuple[EvidenceId,...]=()`; `assumption_ids: tuple[AssumptionId,...]=()`; `limitation_ids: tuple[LimitationId,...]=()` |
...
`BatchEvaluationResult` is frozen/slotted and contains: `batch_id: BatchId`; `batch_fingerprint: BatchFingerprint`; `gate_decisions: tuple[GateDecision,...]` exactly one per result ordered by result ID; `propagated_claim_contexts: tuple[PropagatedClaimContext,...]` exactly one per batch claim ordered by claim ID; `contradiction_records: tuple[ContradictionRecord,...]` exactly one per input ordered by contradiction ID; `derived_limitations: tuple[DerivedLimitationRecord,...]` unique by canonical preimage and ordered by limitation ID; `exclusions: tuple[ExclusionRecord,...]` unique by canonical preimage and ordered by exclusion ID; `synthesis_manifest: SynthesisEligibilityManifest`; and `execution_readiness: ExecutionReadiness` exactly `PLANNING_ONLY`. Every nested output carries identical batch ID/fingerprint. References resolve against caller records in the validated batch or derived records in this aggregate, never object identity. Propagated contexts are derived/output-only and excluded from the caller fingerprint.

### Exact GateDecision and manifest source sets

For each result, `evidence_ids` is the union of every declared evidence ID and every effective evidence ID in the propagated context of each claim owned by the result. `assumption_ids` uses the equivalent declared-plus-effective union. `limitation_ids` is the union of every declared caller limitation, every effective limitation ID in those propagated contexts, and every derived limitation whose related results contain it. `contradiction_ids` is every contradiction whose left or right claim belongs to it. Each is unique lexical and may be empty. Cross-result inherited IDs resolve against the single batch-owned caller record namespace; they do not transfer record ownership or mutate the source result.
...
Manifest `evaluated_result_ids` is all batch results once, lexical. `accepted_result_ids` is exactly final `PASS`/`PASS_WITH_LIMITATIONS`. `accepted_claim_ids` unions accepted-claim IDs from accepted results. `limitation_ids` unions the complete `GateDecision.limitation_ids` source set from accepted results only, including propagated material and non-material limitation IDs. `unresolved_contradiction_ids` is exactly unresolved/incomparable records touching an accepted result. All are unique lexical.
...
`ORIGINAL` requires no parents. `REPEATS`, `REFORMULATES` and `DERIVES` require one or more resolved parent claims. The batch lineage graph must be acyclic; a self-reference or direct/indirect cycle raises `QualityGateContractError` after parent resolution and before propagation. No text similarity exists. Repeated/reformulated confidence cannot exceed every parent's effective confidence; multi-parent derived confidence cannot exceed the minimum effective parent confidence under the canonical order. If any parent is `UNKNOWN`, the ceiling is `UNKNOWN`. Original claims retain caller confidence, which gates do not certify. New evidence is retained but never permits an increase. Evidence independence remains deferred.

For every claim, propagation recursively unions its own evidence, assumption and limitation IDs with all effective parent sets and emits one `PropagatedClaimContext`. Both material and non-material limitation identities propagate without conversion or loss. An effective material limitation participates in the owning result's final gate derivation exactly like a locally referenced material limitation. Multiple paths to the same batch record deduplicate by stable ID; an attempted merge of the same ID with unequal normalized record content raises `QualityGateContractError`. Parent order cannot affect the lexical output. Re-evaluating an equal batch yields equal contexts, decisions, aggregate and manifest. Propagation is performed inside `QualityGateEvaluator` after reference/DAG validation and before contradiction adjustment and final gate/manifest construction; it is not a standalone unused helper. No caller record is mutated.
...
Accepted-gate precedence: (1) `SCOPE_COMPLETE` or `SUFFICIENT_EVIDENCE` -> `STOP`; (2) `MATERIAL_FINDING`, `DEPENDENCY_INVALIDATED` or `REVERSIBLE_TEST_HIGHER_VALUE` -> `REPLAN_REQUIRED`; (3) `DIMINISHING_VALUE`, `TOOL_LIMIT_REACHED` or `CAPABILITY_LIMIT_REACHED` -> `STOP`; (4) no trigger -> `CONTINUE_CURRENT_PLAN`. Precedence selects only `decision`: every validated caller replan reason remains in `DecisionResult.replan_reasons` and every validated caller stop reason remains in `DecisionResult.stop_reasons`, including lower-tier reasons, each unique in enum-value order. Thus `SCOPE_COMPLETE` + `MATERIAL_FINDING` + `DIMINISHING_VALUE` returns `STOP`, retains `MATERIAL_FINDING`, and retains both stop reasons in normative order. Any incompatible combination raises.
