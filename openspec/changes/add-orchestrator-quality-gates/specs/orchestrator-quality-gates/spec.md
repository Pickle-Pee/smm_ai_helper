...
#### Scenario: Controlled caller construction
- **WHEN** any public caller-owned contract receives a missing required field, unknown field, conflicting positional/keyword value, or derived/output-only field
- **THEN** it raises `QualityGateContractError` rather than autogenerated argument-binding `TypeError`
- **AND** valid declared positional and keyword construction remains frozen, slotted and deeply immutable
...
The system SHALL propagate only through a resolved acyclic graph of explicit lineage IDs, recursively union evidence/assumptions/material and non-material limitations by ID, reject conflicting identities, and emit stable lexical ordering without text similarity. It SHALL expose exactly one output-only propagated context per claim in the batch aggregate and SHALL include those effective IDs in the owning result's `GateDecision` and accepted-result manifest source sets defined by the design.

#### Scenario: Propagated output ownership
- **WHEN** a claim inherits records from one or more same-batch parent claims, including parents in another result
- **THEN** its immutable aggregate context contains the recursive effective IDs and conservative effective confidence
- **AND** the owning result decision contains the propagated evidence, assumption and limitation IDs
- **AND** an effective material limitation participates in final gate derivation
- **AND** accepted-result propagated limitations flow into the manifest without mutating or transferring caller record ownership

#### Scenario: Deterministic graph handling
- **WHEN** parent order or repeated paths change without changing normalized lineage meaning
- **THEN** propagation deduplicates identical IDs, orders them lexically and produces an equal evaluation
- **AND** unequal normalized content for one ID or a lineage cycle raises `QualityGateContractError`
...
- **THEN** `STOP` wins
- **AND** every validated stop and replan reason remains in its corresponding output tuple in normative order
- **AND** duplicate list/tuple triggers are rejected while reordered set-like inputs produce the same tuples
