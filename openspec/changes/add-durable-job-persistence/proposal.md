...
- Define aggregate-child cascade deletion, explicit non-audit retention, a closed `pending -> running -> succeeded|failed` lifecycle, bounded structured payload/result/error rules, deterministic timestamps, exact database predicates, and only query-backed indexes.
- Add a caller-transaction-owned internal persistence service for creation, validated lookup, run-scoped listing, and locked lifecycle transitions. It deep-copies request data, owns an injected UTC clock, may flush, never refreshes mutation results, and never commits or rolls back.
- Define request identity/input as immutable through the supported service boundary, while explicitly leaving direct SQLAlchemy/session writes unsupported rather than introducing triggers or a repository-wide ORM immutability framework.
...
- **Persistence:** one future `jobs` table and one new Alembic revision; no existing table or historical row is rewritten. Direct-user and run-owned Jobs use `ON DELETE CASCADE`; these are operational records, not retained audit history.
...
- **Security and data limits:** only trusted internal application code may create system Jobs. Payloads/results use an exact bounded JSON domain, producers exclude secrets and unnecessary PII, and failed transitions accept only caller-supplied sanitized strings.
...
- Database triggers, a universal ORM immutability layer, or support for direct session mutation of immutable Job fields.
