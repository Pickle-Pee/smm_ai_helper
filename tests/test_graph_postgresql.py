...
def test_persisted_registry_version_executes_after_process_recreation(mvp_database, version):
    ...
    async def exercise():
        ...
        if version == "1.1.0":
            # Frozen by the original merge d84d316 compiler, not regenerated by v2 code.
            import json
            from pathlib import Path
            from app.orchestration_runtime.serialization import plan_from_json
            plan = plan_from_json(json.loads((Path(__file__).parent / "fixtures/compiled_execution_plan_v1.json").read_text()))
        else:
